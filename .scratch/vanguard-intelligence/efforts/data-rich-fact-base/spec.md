...
- **Internet-source collection with audit controls**: primary regulatory/company sources are preferred; aggregators may locate evidence but publication requires a primary source or an independent corroborating public source.
...
9. As a consultant, I want an aggregator-sourced lead corroborated before publication, so that convenient discovery never becomes unsupported evidence.
...
- **Evidence policy**: primary regulatory filings, regulated annual reports, and issuer-hosted disclosures take precedence. Public aggregators may be discovery inputs only; a published observation requires a primary source or an independent corroborating public source. Unavailability is recorded as a gap, never inferred from scraper failure.
